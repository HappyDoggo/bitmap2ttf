...
def outliner(orig_image):
    ...
    while True:
        ...
        while not done:
            (x,y) = list(map(sum, list(zip((px,py),ns[d]))))
            (x1,y1) = list(map(sum, list(zip((px,py),ns[(d+3)%4]))))
            nextr = data[x,y]
            nextl = data[x1,y1]
            if nextl == nextr:
                polygon.append((px,py))
                ...
                (px,py) = list(map(sum, list(zip((px,py),ds[d]))))

            else:
                if nextl > 0:   # left white, right black - go straight
                    (px,py) = list(map(sum, list(zip((px,py),ds[d])))) # dont append
        ...
        polygons.append([tuple(map(sum, list(zip(x,(-1,-1))))) for x in polygon])
